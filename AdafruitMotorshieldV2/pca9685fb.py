...
class PCA9685:
  ...
  def __init__(self, i2c, address = 0x60):
    ...
    On the ESP32, bus 0 has SCL on pin 18 and SDA on pin 19
                  bus 1 has SCL on pin 25 and SDA on pin 26
                  SoftI2C buses can use any pair of usable pins.
    """
    self._i2c = i2c
    # Check for I2C from module pyb or from module machine
    self._pyb_i2c = "mem_write" in i2c.__class__.__dict__
    self._address = address
    self.tmpbuf = bytearray(1)
  ...
  def _write(self, data, register):
    ...
    if self._pyb_i2c:
      ...
      self.tmpbuf[0] = data
      self._i2c.writeto_mem(self._address, register, self.tmpbuf)
  ...
  def _setBit(self, bit, register):
    ...
    self._write((curval[0] | bit).to_bytes(1, sys.byteorder)[0], register)
  ...
  def reset(self):
    """
    Reset the PCA9685 by writing 6 at address 0 (in fact all PCA9685 connected to 
    the I2C bus are reset by this call).
    See 'Software reset' in the PCA9685 documentation.
    """
    if self._pyb_i2c:
      self._i2c.send(0x06, 0x00)
    else:
      self._i2c.writeto(0x00, b'\x06')
