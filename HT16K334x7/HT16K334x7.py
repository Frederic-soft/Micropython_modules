...
class HT16K334x7:
  ...
  def displayDigit(self, dig_num, value, dot=False):
    """
    Display a value on digit dig_num.
    If 'value' is negative, the opposite will be displayed.
    If 'value' is greater than 9, a hyphen will be displayed.
    If 'dot' is True, the decimal dot will be on.
    Digits are numbered 0 to 3 from left to right
    """
    self.stopDisplayString()
    if dot:
      dotbit = HT16K334x7.decimalPointOn
    else:
      dotbit = HT16K334x7.decimalPointOff
      
    if value < 0:
      value = -value
    if value > 9:
      self.display(dig_num, hyphen | dotbit)
    else:
      self.display(dig_num, HT16K334x7.digitTable[value] | dotbit)
  
  def displayNumber(self, value, dot=None):
    """
    Display a number.
    ...
    If dot is not None, the decimal dot of digit "dot" (0..3) will be on.
    """
    self.stopDisplayString()
    if value < 0:
      alldots = True
    ...
    if value > 9999:
      for dig in range(4):
        self.display(dig, HT16K334x7.hyphen | (HT16K334x7.decimalPointOn if alldots else HT16K334x7.decimalPointOff))
    else:
      for dig in range(3, -1, -1):
        self.displayDigit(dig, value % 10, alldots or (not dot is None and dot == dig))
        value = value // 10
